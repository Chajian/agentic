...
const arbCategory = fc.stringOf(fc.constantFrom(...'abcdefghijklmnopqrstuvwxyz'.split('')), {
  minLength: 1,
  maxLength: 15,
});
...
const arbDocumentWithTerms = (terms: string[]): fc.Arbitrary<DocumentInput> => {
  return fc.record({
    ...
    content: fc
      .tuple(arbNonEmptyString, fc.subarray(terms, { minLength: 0 }))
      .map(([base, selectedTerms]) => `${base} ${selectedTerms.join(' ')} additional content`),
    metadata: fc.constant(undefined),
  });
};
      ...
      fc.asyncProperty(
        ...
        async (documents, queryTerms) => {
          ...
          for (const result of results) {
            const content =
              `${result.document.title ?? ''} ${result.document.content}`.toLowerCase();
    ...
    fc.assert(
      fc.property(
        fc.record({
          ...
          requiredTopics: fc.option(fc.array(arbNonEmptyString, { minLength: 1, maxLength: 3 }), {
            nil: undefined,
          }),
        }),
        (intent) => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.record({
          action: arbNonEmptyString,
          entities: fc.array(arbNonEmptyString, { minLength: 0, maxLength: 3 }),
          requiredTopics: fc.option(fc.array(arbNonEmptyString, { minLength: 0, maxLength: 3 }), {
            nil: undefined,
          }),
        }),
        arbNonEmptyString,
